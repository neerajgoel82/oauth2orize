...
module.exports = function(options, issue) {
  ...
  if (!issue) { throw new TypeError('oauth2orize.facebookTaken exchange requires an issue callback'); }
  ...
  return function facebookToken(req, res, next) {
    ...
    if (!facebookToken) { return next(new TokenError('Missing required parameter: facebookToken', 'invalid_request')); }
    ...
    try {
      ...
      if (arity == 5) {
        issue(client, username, facebookToken, scope, issued);
      } else { // arity == 4
        issue(client, username, facebookToken, issued);
      }
    } catch (ex) {
      return next(ex);
    }
